...
# Shaders the plugin assembles at run time. Bassalt has none.
ASSEMBLED = {}

named, unnamed = {}, []
for f in FILES:
	text = pathlib.Path( f ).read_text()
	# A shader is one raw string or several adjacent ones -- MSVC C2026 caps a
	# literal at about 16 KB, so the update pass is two, and they are joined
	# here exactly as the compiler joins them.
	for m in re.finditer( r'(\w+)\s*=\s*((?:R"\(.*?\)"\s*)+);', text, re.S ):
...
step "Bundle"
#---------------------------------------------------------------------------
bundle="$BUILD/Bassalt.bundle"
binary="$bundle/Contents/MacOS/Bassalt"
...
echo "ok   Bassalt: $arches, plugMain exported"
...
[[ "$identifier" == "com.stoatworks.ffgl.bassalt" ]] || fail "bundle id is '$identifier'"
[[ "$executable" == "Bassalt" ]] || fail "CFBundleExecutable is '$executable'"
...
if [[ -x "$OXBOW" ]]; then
	...
	printf '%s\n' "$probe" | sed 's/^/   /'
	grep -q 'BS01' <<<"$probe" || fail "oxbow did not read the id BS01"
	grep -q 'SW Bassalt' <<<"$probe" || fail "oxbow did not read the name 'SW Bassalt'"
	grep -qi 'effect' <<<"$probe" || fail "oxbow did not read the type as an effect"
	echo "ok   a host reads BS01 / SW Bassalt / effect"
else
...
"$BUILD/bstest" --negative || fail "a negative control passed"

#---------------------------------------------------------------------------
step "Mutation"
#---------------------------------------------------------------------------
# One character of the shipped GLSL changed: the harness must notice, which is
# the proof that it drives the shipped shaders and not a copy of them.
"$BUILD/bstest" --mutate || fail "the mutated shader went unnoticed"
