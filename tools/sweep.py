...
# Applied to every render: nothing. The default lamp starts warm and moving.
BASE = []

# What else has to be true for a parameter to be able to do anything, as raw
# harness arguments.
BEAT = ["--beat"]
BASS = ["--beat", "--set", "Bass=1"]
CONTEXT = {
    "Bass": BEAT,
    "Bass Band": BASS,
    "Kick": BEAT,
...
def render(harness, out, settings, raw, verbose):
    args = [str(harness), "--out", str(out), "--size", "480x270", "--frames", "60"]
    args += raw
    ...
    if result.returncode != 0:
        raise RuntimeError(f"bstest failed: {result.stderr.strip()}")
...
def parameters(harness):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"bstest --list failed: {result.stderr.strip()}")
...
def main():
    ...
    harness = REPO / args.build / "bstest"
    if not harness.exists():
        print(f"no bstest at {harness} -- build first", file=sys.stderr)
        return 2
